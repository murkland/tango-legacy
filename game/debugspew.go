...
func (g *Game) spewDebug(screen *ebiten.Image) {
	...
	if g.match != nil && g.match.battle != nil {
		lines = append(lines,
			...
			fmt.Sprintf("inputq:  %2d vs %2d (max %2d)", g.match.battle.iq.qs[g.match.battle.LocalPlayerIndex()].Used(), g.match.battle.iq.qs[g.match.battle.RemotePlayerIndex()].Used(), g.runaheadTicksAllowed()),
		)
	}
